...
def main(configs : omegaconf.DictConfig, BSZ):
    ...
    # Make Directories
    logging_dir = "exp_2/log"
    if os.path.exists(logging_dir) == False:
        os.makedirs(logging_dir)
    model_dir = "exp_2/model"
    ...
    wandb.init(project='nlp_pretrain_exp2',
               name=f'[imdb] BSZ-{BSZ}',
    ...
    for epoch in range(num_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            train_loss = outputs['loss'] / ACCUM_STEP
            train_loss.backward()
            if (step+1) % ACCUM_STEP == 0 or (step + 1) == len(train_dataloader):
                optimizer.step()
                optimizer.zero_grad()
            counter.update(1)
            if step % 100 == 0:
        ...
        torch.save(model.state_dict(), f'exp_2/model/BSZ-{BSZ}_{epoch+1}.pt')
        print('model saved!')

    # test after training
    with torch.no_grad():
        model.load_state_dict(torch.load(f'exp_2/model/BSZ-{BSZ}_{best_acc_epoch}.pt'))
        print('model loaded!')
        model.eval()
        test_loss = 0
        test_step = 0
